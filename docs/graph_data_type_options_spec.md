...
- `layout_assets`
  - `grip_layouts` (explicit per-`k` 3D layout RDS files generated by `grip.layout()`, used by RGL/Plotly)
  - `grip_layout_params` (optional generation metadata: rounds/num_init/etc.)
  - `variants` (HTML paths indexed by renderer/layout/size/color presets)
