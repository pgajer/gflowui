app_server <- function(input, output, session) {
  ...
  basin_display_settings <- shiny::reactiveValues(
    ...
    inspector_scope = "initial_display",
    inspector_sort = "mass",
  ...
  basin_tree_event_metrics$shell_render_count <- 0L
  ...
  reset_basin_tree_interaction <- function() {
    ...
    basin_tree_interaction$basin_vertex_color <- "#2563EB"
  ...
  shiny::observeEvent(input$basin_extrema_max_scope, {
    ...
    if (scope %in% c("none", "selected", "initial_display", "all")) {
  ...
  basin_tree_graph_overlay <- shiny::reactive({
    ...
    if (length(active)) {
      ...
      basin.vertex.color <- as.character(
        basin_tree_interaction$basin_vertex_color %||% "#2563EB"
      )
      if (length(basin.vertex.color) != 1L ||
          is.na(basin.vertex.color) ||
          !nzchar(basin.vertex.color)) {
        basin.vertex.color <- "#2563EB"
      }
      displayed.component.colors <- tree$component.colors
      if (identical(
        as.character(
          basin_tree_interaction$component_colors %||% "distinct"
        ),
        ...
        displayed.component.colors[] <- basin.vertex.color
      }
      component.colors <- unname(
        displayed.component.colors[tree$membership$component.id]
      )
    ...
    basin.vertex.size <- suppressWarnings(as.numeric(
      basin_tree_interaction$basin_vertex_size %||% 1
    ))
    ...
    ascent.edges <- .gflowui_basin_empty_ascent_flow_edges()
    if (show.ascent.connections && length(active)) {
      result <- basin_result()
      if (!is.list(result) ||
          !inherits(result$basin, "basin_complex")) {
        stop(
          "The canonical trajectory-flow basin complex is unavailable.",
          call. = FALSE
        )
      }
      ascent.edges <- gflowui_basin_ascent_flow_edges(
        trajectory.complex = result$basin,
    ...
    overlay <- list(
      ...
      component.colors = displayed.component.colors,
  ...
  basin_displayed_table <- function(
      result,
      state = shiny::isolate(basin_analysis_state()),
      selected.keys = shiny::isolate(basin_selected_keys())) {
    gflowui_basin_inspector_rows(
      result,
      state = state,
      scope = basin_display_settings$inspector_scope %||%
        "initial_display",
      sort.by = basin_display_settings$inspector_sort %||% "mass",
  ...
  basin_extrema_scope_rows <- function(result, direction, scope) {
    ...
    table <- gflowui_basin_inspector_rows(
      result,
      state = basin_analysis_state(),
      scope = if (identical(scope, "all")) {
        if (identical(direction, "max")) "all_maxima" else "all_minima"
      } else if (identical(scope, "initial_display")) {
        "initial_display"
  ...
  output$basin_inspector_ui <- shiny::renderUI({
    ...
    table <- basin_displayed_table(result, analysis, selected)
    inspector.scope <- as.character(
      basin_display_settings$inspector_scope %||% "initial_display"
    )
    inspector.sort <- as.character(
      basin_display_settings$inspector_sort %||% "mass"
    )
    colors <- basin_color_map()
    ...
    header <- shiny::tags$tr(
      shiny::tags$th(
        ...
        title = "Transiently select/show without changing proposal membership.",
        "Show"
      ),
      shiny::tags$th(
        class = "gf-basin-pin-column",
        title = "Protect this maximum basin and reconstruct the proposal.",
        "Pin"
      ),
      ...
      shiny::tags$th(
        ...
        title = "Stable canonical ID used to link all Basin Analysis panels.",
        "Canonical ID"
      ),
      ...
      shiny::tags$th(
        title = paste(
          "All reasons for proposal inclusion, including core, sentinels,",
          "pins, component survivor, and ancestor closure."
        ),
        "Inclusion reasons"
      ),
      shiny::tags$th(
        title = "Visible or hidden in the current filtered maximum-basin tree.",
        "Tree state"
      ),
      shiny::tags$th(
        class = "gf-basin-full-column",
    ...
    rows <- lapply(seq_len(nrow(table)), function(index) {
      ...
      shiny::tags$tr(
        ...
        shiny::tags$td(
          class = "gf-basin-pin-column",
          if (identical(as.character(row$type), "max") &&
              isTRUE(row$proposal.component)) {
            shiny::tagAppendAttributes(
              shiny::checkboxInput(
                paste0("basin_pin_", suffix),
                label = NULL,
                value = isTRUE(row$proposal.pinned)
              ),
              `aria-label` = sprintf("Pin %s", row$display.label),
              `data-gf-basin-key` = key,
              `data-gf-basin-role` = "pin"
            )
          } else {
            shiny::span(
        ...
        shiny::tags$td(
          class = "gf-basin-full-column gf-basin-canonical-id",
          as.character(row$canonical.basin.id)
        ),
        if (show.extremum.vertex) {
        ...
        shiny::tags$td(
          if (nzchar(as.character(row$proposal.inclusion.reasons))) {
            as.character(row$proposal.inclusion.reasons)
          } else {
            "\u2014"
          }
        ),
        shiny::tags$td(as.character(row$proposal.visibility)),
        shiny::tags$td(
          class = "gf-basin-full-column",
    ...
    shiny::tags$section(
      ...
      shiny::div(
        ...
        shiny::selectInput(
          "basin_inspector_scope",
          "Row scope",
          choices = gflowui_basin_inspector_scope_choices(),
          selected = inspector.scope,
          width = "190px"
        ),
        shiny::selectInput(
          "basin_inspector_sort",
          "Row sort",
          choices = gflowui_basin_inspector_sort_choices(),
          selected = inspector.sort,
          width = "170px"
        ),
        shiny::selectInput(
        ...
        shiny::p(
          ...
          sprintf(
            paste(
              ...
              "Row scope, sorting, selections, and display filters are ignored.",
  ...
  basin_plot_scope_choices <- c(
    "All basins" = "all",
    "Selected-component maxima" = "component_maxima",
    "Initial display" = "initial_display",
    "Core" = "core",
    "Sentinels" = "sentinels",
    "Pinned" = "pinned",
  ...
  basin_plot_point_color_choices <- c(
    "By proposal membership" = "proposal",
  ...
  output$basin_merge_tree_ui <- shiny::renderUI({
    basin_tree_event_metrics$shell_render_count <-
      basin_tree_event_metrics$shell_render_count + 1L
    result <- basin_result()
    state <- basin_analysis_state()
    if (!isTRUE(basin_inspector_open()) ||
        !is.list(result) ||
    ...
    model <- tryCatch(
      shiny::isolate(basin_merge_tree_model_for_state(state, result)),
      error = function(error) error
    )
    ...
    interaction.settings <- shiny::isolate(list(
      continuation.rule = as.character(
        basin_tree_interaction$continuation_rule %||% "field_value"
      ),
      scope = as.character(
        basin_tree_interaction$scope %||% "proposal"
      ...
      basin.vertex.size = as.numeric(
        basin_tree_interaction$basin_vertex_size %||% 1
      ),
      link.graph = isTRUE(basin_tree_interaction$link_graph),
      show.ascent.connections = isTRUE(
        basin_tree_interaction$show_ascent_connections
      ),
      connection.color.mode = as.character(
      ...
      event.index = as.integer(
        basin_tree_interaction$event_index %||% 0L
      )
    ))
    interaction.domain <- tryCatch(
      shiny::isolate(basin_tree_event_domain()),
      error = function(error) NULL
    )
    ...
    interaction.index <- suppressWarnings(as.integer(
      interaction.settings$event.index %||% 0L
    ))
    ...
    shiny::tags$section(
      ...
      shiny::tags$details(
        ...
        .gflowui_basin_panel_controls_help(
          interaction.settings$continuation.rule
        ),
        shiny::div(
      ...
      shiny::tags$section(
        ...
        shiny::div(
          ...
          shiny::selectInput(
            ...
            selected = interaction.settings$scope
          ),
          shiny::selectInput(
            ...
            selected = interaction.settings$component.colors
          ),
          shiny::checkboxInput(
            "basin_tree_link_graph",
            "Link h to the 3D graph",
            value = interaction.settings$link.graph
          ),
          shiny::selectInput(
            ...
            selected = interaction.settings$merge.scope
          )
        ),
        shiny::tags$details(
          class = "gf-basin-tree-style-controls",
          shiny::tags$summary(
            "Basin, maximum, and merge-plateau styling"
          ),
          shiny::div(
            class = "gf-basin-tree-control-grid",
            shiny::selectInput(
              "basin_tree_basin_vertex_color",
              "Basin vertex color (one-color mode)",
              choices = basin_color_choices(
                interaction.settings$basin.vertex.color
              ),
              selected = interaction.settings$basin.vertex.color
            ),
            shiny::sliderInput(
              ...
              max = 3,
              step = 0.1,
              value = interaction.settings$basin.vertex.size
            ),
            shiny::checkboxInput(
              "basin_tree_show_maxima_labels",
              "Label active maxima",
              value = interaction.settings$show.maxima.labels
            ),
            shiny::selectInput(
              "basin_tree_maxima_color",
              "Maximum color",
              choices = basin_color_choices(
                interaction.settings$maxima.color
              ),
              selected = interaction.settings$maxima.color
            ),
            shiny::sliderInput(
              ...
              max = 3,
              step = 0.1,
              value = interaction.settings$maxima.size
            ),
            shiny::sliderInput(
              ...
              max = 3,
              step = 0.1,
              value = interaction.settings$maxima.label.size
            ),
            shiny::checkboxInput(
              "basin_tree_show_merge_labels",
              "Label merge plateaus",
              value = interaction.settings$show.merge.labels
            ),
            shiny::selectInput(
              "basin_tree_merge_color",
              "Merge-plateau color",
              choices = basin_color_choices(
                interaction.settings$merge.color
              ),
              selected = interaction.settings$merge.color
            ),
            shiny::sliderInput(
              ...
              max = 3,
              step = 0.1,
              value = interaction.settings$merge.size
            ),
            shiny::sliderInput(
              ...
              max = 3,
              step = 0.1,
              value = interaction.settings$merge.label.size
            )
          ),
          shiny::conditionalPanel(
            condition = "input.basin_tree_link_graph",
            shiny::div(
              class = "gf-basin-tree-connection-controls",
              shiny::h6("Canonical ascent-flow connections"),
              shiny::checkboxInput(
                "basin_tree_show_ascent_connections",
                paste(
                  "Show each active nonmaximum vertex's next",
                  "CLOSEST-ascent connection"
                ),
                value = interaction.settings$show.ascent.connections
              ),
              shiny::conditionalPanel(
                condition = "input.basin_tree_show_ascent_connections",
                shiny::div(
                  class = "gf-basin-tree-control-grid",
                  shiny::selectInput(
                  ...
                  shiny::sliderInput(
                    ...
                    max = 1,
                    step = 0.05,
                    value = interaction.settings$connection.opacity
                  ),
                  shiny::sliderInput(
                    ...
                    max = 8,
                    step = 0.25,
                    value = interaction.settings$connection.width
                  )
                )
              )
            )
  ...
  shiny::observeEvent(input$basin_tree_basin_vertex_color, {
  ...
  shiny::observeEvent(input$basin_tree_basin_vertex_size, {
    value <- suppressWarnings(as.numeric(
      input$basin_tree_basin_vertex_size
    ))
    if (is.finite(value)) {
      basin_tree_interaction$basin_vertex_size <-
        max(0.5, min(3, value))
    }
  }, ignoreInit = TRUE)
  ...
  shiny::observeEvent(input$basin_tree_show_ascent_connections, {
    basin_tree_interaction$show_ascent_connections <- isTRUE(
      input$basin_tree_show_ascent_connections
    )
  }, ignoreInit = TRUE)
  ...
  shiny::observeEvent(input$basin_tree_connection_opacity, {
    value <- suppressWarnings(as.numeric(
      input$basin_tree_connection_opacity
    ))
    if (is.finite(value)) {
  ...
  shiny::observeEvent(input$basin_tree_connection_width, {
    value <- suppressWarnings(as.numeric(
      input$basin_tree_connection_width
    ))
    if (is.finite(value)) {
  ...
  shiny::observeEvent(basin_selected_keys(), {
    ...
    selected <- gflowui_basin_selected_canonical_ids(
      result,
      state,
  ...
  shiny::observeEvent(input$basin_inspector_scope, {
    value <- as.character(
      input$basin_inspector_scope %||% "initial_display"
    )
    if (value %in% unname(gflowui_basin_inspector_scope_choices())) {
      basin_display_settings$inspector_scope <- value
    }
  }, ignoreInit = TRUE)

  shiny::observeEvent(input$basin_inspector_sort, {
    value <- as.character(input$basin_inspector_sort %||% "mass")
    if (value %in% unname(gflowui_basin_inspector_sort_choices())) {
      basin_display_settings$inspector_sort <- value
    }
  }, ignoreInit = TRUE)
  ...
  shiny::observeEvent(input$basin_inspector_row_event, {
    ...
    if (!nzchar(key) || !(key %in% as.character(result$all_table$key))) {
      return()
    }
    if (identical(role, "pin")) {
      state <- shiny::isolate(basin_analysis_state())
      if (!is.list(state)) {
        return()
      }
      ...
      row <- context[as.character(context$key) == key, , drop = FALSE]
      if (nrow(row) != 1L ||
          !identical(as.character(row$type), "max") ||
          !isTRUE(row$proposal.component)) {
        return()
      }
      canonical.id <- as.character(row$canonical.basin.id)
      checked <- isTRUE(event$checked)
      already.pinned <- canonical.id %in% state$pinned.ids
      if (identical(checked, already.pinned)) {
        return()
      }
      apply_basin_analysis_event(
        gflowui_basin_state_event(
          if (checked) "pin" else "unpin",
          id = canonical.id
        ),
        sprintf(
          "%s %s and updated Basin Analysis.",
          if (checked) "Pinned" else "Unpinned",
          canonical.id
        )
      )
      return()
    ...
    next.state <- gflowui_update_basin_row_state(
      ...
      valid_keys = result$all_table$key,
      key = key,
      role = role,
  ...
  if (requireNamespace("plotly", quietly = TRUE)) {
    ...
    output$reference_plot <- plotly::renderPlotly({
      ...
      if (isTRUE(tree.linked)) {
        ascent.flow <- tree.overlay$ascent.flow
        if (is.list(ascent.flow) &&
        ...
        inactive.rows <- !(
          plot_data$vertex %in% tree.overlay$active.vertices
        )
        if (any(inactive.rows)) {
          p <- p %>%
            plotly::add_trace(
              type = "scatter3d",
              mode = "markers",
              x = plot_data$x[inactive.rows],
              y = plot_data$y[inactive.rows],
              z = plot_data$z[inactive.rows],
              key = plot_data$vertex[inactive.rows],
              customdata = plot_data$vertex[inactive.rows],
              name = "Below h",
              text = sprintf(
                "vertex=%d<br>field value &lt; h",
                plot_data$vertex[inactive.rows]
              ),
              hoverinfo = "text",
              marker = list(
                size = point_size,
        ...
        maximum.vertices <- suppressWarnings(as.integer(
          tree.overlay$tree$maxima$extremum.vertex
        ))
        for (component.index in seq_len(nrow(components))) {
          ...
          vertices <- setdiff(vertices, maximum.vertices)
          if (!length(vertices)) {
            next
          }
          ...
          color <- unname(
            tree.overlay$component.colors[component.id]
          )
          p <- p %>%
            plotly::add_trace(
              type = "scatter3d",
              ...
              hoverinfo = "text",
              marker = list(
                size = point_size * tree.overlay$basin.vertex.size,
                color = color,
    ...
    shiny::observe({
      ...
      if (isTRUE(tree.linked)) {
        ...
        if (is.list(ascent.flow) &&
            ...
            is.data.frame(ascent.flow$edges) &&
            nrow(ascent.flow$edges)) {
          from.view <- match(ascent.flow$edges$from, keep_idx)
          to.view <- match(ascent.flow$edges$to, keep_idx)
          keep.connections <- is.finite(from.view) &
            is.finite(to.view) &
          ...
          if (any(keep.connections)) {
            ...
            tree_layers[[length(tree_layers) + 1L]] <- list(
              fun = function(
                  ...
                  connection_colors,
                  connection_width) {
                segments <- as.matrix(connection_segments)
                if (!nrow(segments)) {
                  return(invisible(NULL))
                }
                vertices <- as.vector(t(segments))
                rgl::segments3d(
                  ctx$X[vertices, , drop = FALSE],
                  col = rep(connection_colors, each = 2L),
                  lwd = max(0.5, connection_width),
                  lit = FALSE
                )
                invisible(NULL)
              },
        ...
        basin.view <- setdiff(which(active.view), maxima.view)
        if (length(basin.view)) {
          tree_layers[[length(tree_layers) + 1L]] <- list(
            fun = function(
                ctx,
                basin_idx,
              ...
              keep <- is.finite(idx) &
                idx >= 1L & idx <= nrow(ctx$X)
              idx <- idx[keep]
              colors <- as.character(basin_colors)[keep]
              if (!length(idx)) {
                return(invisible(NULL))
              }
              colors <- grDevices::adjustcolor(
                colors,
                alpha.f = basin_opacity
              )
              if (identical(draw_mode, "sphere")) {
                rgl::spheres3d(
                  ctx$X[idx, , drop = FALSE],
                ...
                rgl::points3d(
                  ctx$X[idx, , drop = FALSE],
                  col = colors,
                  size = max(1.2, marker_size * basin_size)
                )
              }
              invisible(NULL)
            },
            args = list(
              basin_idx = basin.view,
              basin_colors =
                tree.overlay$vertex.colors[keep_idx[basin.view]],
              basin_size = tree.overlay$basin.vertex.size,
              basin_opacity = tree.overlay$active.opacity,
              draw_mode = vertex_mode,
              marker_radius = sphere_radius,
              marker_size = point_size
            ),
