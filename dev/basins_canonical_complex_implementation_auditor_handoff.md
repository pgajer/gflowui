...
Status: Implementation complete; final validation and repository provenance are
recorded below
...
- `gflow` branch: `main`
- `gflow` implementation commit: `5567e11f18e09140bd9fc11beca2400a3e868280`
- `gflowui` branch: `main`
...
- Requires a finite full-domain field and reviewed, unique external vertex IDs
  before canonical construction or cache access.
- Uses one both-direction `gflow::create.basin.complex()` result with fixed
  ...
  trajectories.
- Keys the cache by graph/field/source identity plus exact gflow build and
  runtime identities.
...
- The final source adjustment was followed by `make check-fast`, which
  completed with the same single feasibility warning.
...
- No-trajectory uncached elapsed time: 9.667 seconds.
- Cache-hit elapsed time: 0.119 seconds.
- No-trajectory object size: 56,401,880 bytes.
- Stored-trajectory object size: 61,890,832 bytes.
- Whole benchmark-process maximum resident set: 779,943,936 bytes.
- Whole benchmark-process peak memory footprint: 613,827,616 bytes.
...
- The HMP graph, layout, occupation-density, and metadata assets were treated
  as upstream fingerprinted inputs and were not edited.
...
- The benchmark does not claim basin-ID equivalence with legacy precomputed
  `trajectory_flow` assets. It verifies the canonical reconstruction and
  unchanged input occupation probability.
